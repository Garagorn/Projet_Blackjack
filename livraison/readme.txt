...
2) Lancer les test
Les rapports de tests sont placées de base dans livraison/cartes/RapportTest ou livraison/blackjack/RapportTest

Il peuvent être généré de nouveau. /!\ Certains tests sont probabilistes et peuvent rarement avoir des erreurs (Comparaison entre deux paquets qui mélangées donne le même résultat).
...
La javadoc est placée de base dans livraison/cartes ou livraison/blackjack

Si vous souhaitez la regénéré : 
i)  dans /thomas-siaghi-tellier/cartes effectuer /thomas-siaghi-tellier/cartes$  ant javadoc
	
	ii) dans /thomas-siaghi-tellier/blackjack effectuer /thomas-siaghi-tellier/blackjack$ ant javadoc
    
    iii) rendez vous désormais dans le dossier thomas-siaghi-tellier/blackjack/Dist/docs/API ou vous trouverez le fichier index.html qui vous enverras sur la javadoc.
